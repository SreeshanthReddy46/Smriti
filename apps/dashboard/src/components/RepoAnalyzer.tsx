...
export default function RepoAnalyzer({ status, privacyMode }: { status: any; privacyMode: boolean }) {
  ...
  const renderNode = (node: FileNode) => {
    ...
    const displayName = (node.name.includes('\\') || node.name.includes('/') || node.name.includes(':'))
      ? maskPath(node.name, privacyMode)
      : node.name;
      ...
      <div key={node.path} className="ml-4 font-medium">
        ...
        >
          {isDir ? (
            <>
              {hasChildren ? (
                isExpanded ? <ChevronDown className="w-4.5 h-4.5 text-slate-400" /> : <ChevronRight className="w-4.5 h-4.5 text-slate-400" />
              ) : (
                <ChevronRight className="w-4.5 h-4.5 text-transparent" />
              )}
