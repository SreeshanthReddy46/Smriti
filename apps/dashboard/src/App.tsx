...
export default function App() {
  ...
  const [privacyMode, setPrivacyMode] = useState<boolean>(() => {
    return localStorage.getItem('smriti_privacy_mode') === 'true';
  });

  const togglePrivacyMode = () => {
    setPrivacyMode(prev => {
      const next = !prev;
      localStorage.setItem('smriti_privacy_mode', String(next));
      return next;
    });
  };
    ...
    <div className="flex h-screen bg-slate-50 overflow-hidden">
      ...
      <main className="flex-1 flex flex-col overflow-hidden bg-white">
        ...
        <header className="h-16 border-b border-slate-100 px-8 flex items-center justify-between">
            ...
            {status.repo_loaded && status.repository ? (
              <div className="text-sm">
                ...
                <span className="text-xs bg-slate-100 text-slate-600 px-2 py-1 rounded font-mono font-medium">
                  {maskPath(status.repository.path, privacyMode)}
                </span>
              </div>
            ) : (
          ...
          <div className="flex items-center gap-4">
            ...
            <button
              ...
              className={`p-2 border rounded-lg transition-colors flex items-center justify-center gap-1.5 text-xs font-bold ${
                privacyMode 
                  ? 'bg-rose-50 border-rose-200 text-rose-600 hover:bg-rose-100 shadow-sm shadow-rose-500/5' 
                  : 'bg-white border-slate-200 text-slate-600 hover:bg-slate-50 hover:text-slate-800'
              }`}
              title={privacyMode ? "Disable Secure Mode (reveals usernames and snippets)" : "Enable Secure Mode (masks usernames and blurs snippets)"}
            >
              {privacyMode ? (
                <>
                  <Shield className="w-4 h-4 text-rose-500 animate-pulse" /> Secure Mode ON
                </>
              ) : (
                <>
                  <ShieldOff className="w-4 h-4" /> Secure Mode OFF
                </>
              )}
            </button>
        ...
        <div className="flex-1 overflow-y-auto p-8 bg-white">
          {activeTab === 'dashboard' && (
            <DashboardHome status={status} fetchStatus={fetchStatus} setActiveTab={setActiveTab} privacyMode={privacyMode} />
          )}
          {activeTab === 'repo' && (
            <RepoAnalyzer status={status} privacyMode={privacyMode} />
          )}
          {activeTab === 'memory' && (
            <MemoryEngine />
          )}
          ...
          {activeTab === 'search' && (
            <SemanticSearch privacyMode={privacyMode} />
          )}
          {activeTab === 'settings' && (
            <Settings 
              status={status} 
