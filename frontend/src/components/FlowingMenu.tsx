...
interface FlowingMenuProps {
  items: Array<{ id: string; link: string; text: string; image: string }>;
  speed?: number;
  textColor?: string;
  bgColor?: string;
  marqueeBgColor?: string;
...
export default function FlowingMenu({
  items = [],
  speed = 15,
  textColor = '#0284c7', // Sky-600 skyblue
...
function MenuItem({ 
  ...
  return (
    <div className="menu__item border-b border-slate-100" ref={itemRef} style={{ borderColor }}>
      <a
        className="menu__item-link font-extrabold"
        href={link}
        onClick={(e) => {
          e.preventDefault();
          onSelect(id);
